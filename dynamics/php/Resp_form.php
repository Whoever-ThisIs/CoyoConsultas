<?php
  ...
  $result = mysqli_query($con, "SELECT pregunta.id_pregunta, pregunta.titulo FROM formulario
  ...
  $Pregunta = [];
  while ($preg = mysqli_fetch_array($result)) {
    //Se optiene el id de esa pregunta
    $id_preg = $preg['id_pregunta'];
    //Se solicita todas las opciones de esa pregunta
    $result = mysqli_query($con, "SELECT opcion.id_pregunta, opcion.id_opcion, opcion.valor FROM formulario
      LEFT JOIN pregunta ON formulario.id_form=pregunta.id_form
      ...
      $Opciones=[];
      while ($opc = mysqli_fetch_array($result)) {
